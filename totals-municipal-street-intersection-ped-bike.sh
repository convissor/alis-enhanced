...
SELECT
	cross_street,
	SUM(crash) AS crash_ped_bike,
	SUM(minor_injury) AS minor_injury_ped_bike,
	SUM(serious_injury) AS serious_injury_ped_bike,
	SUM(fatality) AS fatality_ped_bike
FROM (
	SELECT
	...
	WHERE
		...
		AND veh_type_veh_2 IN ('PEDESTRIAN', 'BICYCLE')
...
sql="$sql
GROUP BY cross_street
ORDER BY crash_ped_bike DESC, cross_street;"
